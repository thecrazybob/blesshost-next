...
export default function simpleHostingPage({ products }) {
  const [billingInterval, setBillingInterval] = useState("year");
  const { currency, setCurrency } = useCurrency("");

  //merging api returned array and hardcoded array
  const plans = planIds.map((item) => {
    //Find items in api array which have same pid as hardcoded array
    const plan = products?.find((product) => product.pid === item.pid);

    //Merge arrays with same pid
    item = { ...plan, ...item };
...
export async function getStaticProps() {
  const { product: products } = await getProducts();
  return {
    props: {
      products,
    },
